...
.product-image-container {
    ...
    overflow: hidden;
    aspect-ratio: 9/16;
    /* Tỷ lệ gần giống điện thoại thực tế */
    height: 650px;
    width: 400px;
    margin: 0 auto;
    display: flex;
...
.product-image-container:hover {
    ...
    z-index: 2;
    /* Tăng nhẹ z-index khi hover nhưng vẫn thấp hơn header */
}
...
.product-image-container::before {
    ...
    pointer-events: none;
    /* Đảm bảo không ảnh hưởng đến tương tác */
}
...
.product-image {
    ...
    position: relative;
    z-index: 2;
    /* Giảm z-index để không đè lên header */
        ...
        <div class="col-lg-6 mb-4">
            <div class="product-image-container">
                <img src="{{ $phone->detail_image_path ? asset($phone->detail_image_path) : ($phone->image_path ? asset($phone->image_path) : asset('images/default-phone.png')) }}"
                    alt="{{ $phone->name }}" class="img-fluid product-image product-image-loading"
                    onload="this.classList.remove('product-image-loading')" onerror="handleImageError(this)">
            </div>
        </div><!-- Product Info -->
        <div class="col-lg-6">
                ...
                <div class="card-body p-4">
                        ...
                        <div class="row align-items-center">
                            <div class="col-8">
                                ...
                                <div class="mb-2">
                                    ...
                                    <span
                                        class="badge bg-danger ms-2 fs-6">-{{ number_format($phone->discount_percentage, 0) }}%</span>
                                </div>
                    ...
                    <form action="{{ route('cart.add', $phone->id) }}" method="POST" id="productForm" class="mb-4">
                            ...
                            <div class="color-options d-flex flex-wrap gap-2"> @php
                                ...
                                <div class="color-option">
                                    <input type="radio" class="btn-check" name="color"
                                        id="color_{{ strtolower($color) }}" value="{{ $color }}"
                                        {{ $color == $phoneColor ? 'checked' : '' }}> <label
                                        class="btn btn-outline-primary color-btn" for="color_{{ strtolower($color) }}">
                                        @if($color == 'Đen')
                                        <span class="color-swatch me-2"
                                            style="background-color: #000000; border: 2px solid #ddd;"></span>
                                        @elseif($color == 'Trắng')
                                        <span class="color-swatch me-2"
                                            style="background-color: #ffffff; border: 2px solid #333;"></span>
                                        @elseif($color == 'Vàng')
                                        <span class="color-swatch me-2"
                                            style="background-color: #ffc107; border: 2px solid #ddd;"></span>
                                        @endif
                                        {{ $color }}
                                    </label>
                                </div>
                        ...
                        <div class="d-flex gap-2 mb-4">
                            <!-- Nút Mua Ngay -->
                            <button type="button" name="buy_now" value="1" class="btn btn-add-to-cart flex-fill"
                                title="Thêm vào giỏ hàng và chuyển thẳng đến trang thanh toán">
                                <i class="fas fa-bolt me-2"></i>MUA NGAY - THANH TOÁN
                            </button>

                            <!-- Nút Thêm vào giỏ hàng -->
                            <button type="button" name="add_to_cart" value="1"
                                class="btn btn-secondary-action flex-fill">
                                <i class="fas fa-cart-plus me-2"></i>Thêm vào giỏ hàng
                            </button>

                            <!-- Nút Yêu thích -->
                            <button type="button" class="btn btn-outline-danger" style="flex: 0 0 auto; width: 50px;">
                    ...
                    <div class="alert alert-info mb-4" role="alert">
                        <div class="d-flex align-items-center">
                            ...
                            <div>
                                ...
                                <p class="mb-0">Để thêm sản phẩm vào giỏ hàng hoặc mua ngay, bạn cần đăng nhập tài
                                    khoản.</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="alert alert-warning border-0 rounded-3"
                        style="background: linear-gradient(45deg, #ffc107, #ffeb3b); border: none;">
                            ...
                            <div class="row mt-4">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <ul class="list-unstyled">
                                        ...
                                        <li><strong>Ngày phát hành:</strong>
                                            {{ \Carbon\Carbon::parse($phone->released_at)->format('d/m/Y') }}</li>
                                    </ul>
                                </div>
                            </div>
                        </div>
                ...
                <div class="col-6 col-md-3 mb-4">
                    <div class="card related-product h-100">
                        <img src="{{ $relatedPhone->image_path ? asset($relatedPhone->image_path) : asset('images/default-phone.png') }}"
                            class="card-img-top" alt="{{ $relatedPhone->name }}"
                        ...
                        <div class="card-body">
                            ...
                            <div class="d-flex justify-content-between align-items-center">
                                ...
                                <a href="{{ route('phones.show', $relatedPhone) }}"
...
// Form validation - simplified inline
    ...
    if (buyNowBtn) {
        buyNowBtn.addEventListener('click', function(e) {
            ...
            if (validateFormData()) {
                ...
                // Set buy_now value và submit form
    ...
    if (addToCartBtn) {
        addToCartBtn.addEventListener('click', function(e) {
            ...
            if (validateFormData()) {
                ...
                // Remove buy_now input nếu có
                const existingBuyNow = productForm.querySelector('input[name="buy_now"]');
                if (existingBuyNow) {
